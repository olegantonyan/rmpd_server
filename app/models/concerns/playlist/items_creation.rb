module Playlist::ItemsCreation
  ...
  included do
    ...
    attr_accessor :media_items_advertising_ids, :media_items_advertising_begin_times, :media_items_advertising_end_times, :media_items_advertising_playbacks_totals

    private

    attr_accessor :playlist_items_background_created, :playlist_items_advertising_created
  end
  ...
  def create_playlist_items_advertising
    ...
    media_items_advertising_ids.each do |i|
      ...
      playbacks_total = media_items_advertising_end_times.find{ |e| e.first.to_i == i.to_i}.second
      begin_time = Time.zone.parse("#{begin_time[:hour]}:#{begin_time[:minute]}")
      end_time = Time.zone.parse("#{end_time[:hour]}:#{end_time[:minute]}")
      playlist_items << Playlist::Item.new(media_item_id: i, playbacks_total: playbacks_total, begin_time: begin_time, end_time: end_time, playlist: self)
    end
